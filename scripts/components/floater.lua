local Floater = Class(function(self, inst) -- mudei a funcao ShouldShowEffect()
    self.inst = inst
...
function Floater:ShouldShowEffect()
	...
	local volcano = #TheSim:FindEntities(x, y, z, 18, { "alt_tile" }) > 0
	
	return  --not TheWorld.Map:IsPassableAtPoint(pos_x, 0, pos_z) and
            not TheWorld.Map:IsVisualGroundAtPoint(pos_x, 0, pos_z) and not plataforma and not volcano
end
