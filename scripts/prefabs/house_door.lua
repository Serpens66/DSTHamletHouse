...
local function common_on_built( inst, builder )
...
local function place_door_test_fn(inst, pt)
    ...
    if current_interior then
        ...
        local ents = TheSim:FindEntities(newpt.x, newpt.y, newpt.z, 2, {}, {}, {"wallsection", "interior_door", "predoor"})
        if #ents >= 1 then
        	for _, ent in pairs(ents) do
...
local function MakeHouseDoor(name, build, bank)
	local function house_fn()
		...
		inst.OnBuilt = function(inst,builder)
			common_on_built(inst,builder)

			if not inst.initialized then
				if inst.animdata~=nil then -- TODO were is inst.animdata defined?
...
local function MakeHouseDoorPlacer(name, build, bank)
	return MakePlacerHamlet( "common/inventory/" .. name .. "_placer", bank, build, name .. "_open_north", nil, nil, nil, nil, nil, nil, nil, nil, nil, place_door_test_fn, modify_house_door, nil)
end
