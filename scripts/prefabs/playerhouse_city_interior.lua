...
local function DoShineFlick(inst)
    inst.AnimState:SetHaunted(true)
        inst:DoTaskInTime(math.random() * 0.65, function()
...
local function ChainPlayerprox(inst, near) --currently doesn't work
    ...
    if link ~= nil and link:IsValid() and not link:IsAsleep() then
        if near then
...
local function PlayTravelSound(inst, doer)
    inst.SoundEmitter:PlaySound("Hamlet/common/objects/store/door_close")
end
...
local function PutTile(px,pz,newType)
    -- sme_setTile(newType, px, 0, pz)
    -- TheWorld.Map:SetTile(px, pz, newType)
end
...
---------------------------------itens de dentro----------------------------
    ...
    if part ~= nil then
    part.Transform:SetPosition(x -4.5 , 0, z -15/3.5)
    PutTile(x -4.5, z -15/3.5, 4)
...
local function OnIsSeasonChange(inst, issummer,iswinter)
    if iswinter then
    inst.components.heater.heat = 115
    elseif issummer then
        inst.components.heater.heat = -37
    else
...
--  inst:AddTag("NOCLICK")
    ...
    -- inst.components.walkableplatform.radius = 15
    -- inst.components.walkableplatform.platform_radius = 15
...
--  OnIsSeasonChange(inst, TheWorld.state.issummer,TheWorld.state.iswinter)
...
--    inst.AnimState:SetOrientation(ANIM_ORIENTATION.OnGround)
    ...
    inst:AddTag("liberado")
    inst:AddTag("HH_houseteleporter") 
...
return  Prefab("playerhouse_city_entrance", entrance),
        ...
        --MakePlacer("common/playerhouse_city_entrance", "pig_house_sale", "pig_house_sale", "idle")