...
local function reconstructed(inst)
    print("RECONSTRUCTED")
        inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/crafted/pighouse/wood_1")
    if ThePlayer.homeowner or not inst:HasTag("citypossession") then
        inst.unboarded = true
        inst.AnimState:Hide("boards")
        inst.components.door.disabled = false 
    end
...
local function onsave(inst, data)
    ...
    if inst:HasTag("fire") then
        ...
        if not inst.IsPlayerInside then
            data.burnt = true
        else
            data.burning = true
        end	
...
local function creatInterior(inst, name)
    if not inst:HasTag("spawned_shop") then
        ...
        if not inst.interiorID then
            ...
            local floortexture = "textures/noise_woodfloor.tex"
            local walltexture = "textures/interiors/shop_wall_woodwall.tex"
            local minimaptexture = "textures/map_interior/mini_ruins_slab.tex"
            local colorcube = "images/colour_cubes/pigshop_interior_cc.tex"

            addprops = {
                { name = "housewall", x_offset =  -3, z_offset =  0 },
...
local function placetestfn(inst)
    ...
    if TheWorld.components.interiorspawner:GetInteriorByPos(pt) then -- not buildable within a room   
        return false
    end
...
return Prefab( "common/objects/playerhouse_city", makefn(), assets, prefabs),
       MakePlacerHamlet("common/playerhouse_city_placer", "pig_house_sale", "pig_house_sale", "idle",nil,nil,nil,0.75,nil,nil,nil,nil,nil, placetestfn)